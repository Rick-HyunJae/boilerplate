...
(function (global) {
    ...
    XHRWrapper.prototype.send = function (body) {
        ...
        if (
            (!('ontimeout' in XMLHttpRequest.prototype) || (!('sendAsBinary' in XMLHttpRequest.prototype) && !('mozAnon' in XMLHttpRequest.prototype))) &&
            document != undefined &&
        ...
        try {
            ...
            /** body 인자가 있는 경우 전송 */
            // xhr.send(undefined);
            xhr.send(body === undefined ? undefined : body);
        } catch (error1) {
    ...
    var contentJsonTypeRegExp = /^application\/json(;.*)?$/i;
    ...
    function start(es, url, options) {
        ...
        var body = options.body === undefined ? undefined : options.body;
        ...
        var onStart = function (status, statusText, contentType, headers) {
            if (currentState === CONNECTING) {
                ...
                } else if (status === 200 && contentType != undefined && contentJsonTypeRegExp.test(contentType)) {
                    // JSON 타입 - 스트리밍 모드로 열지만 JSON으로 처리
                    currentState = OPEN;
                    wasActivity = Date.now();
                    retry = initialRetry;
                    es.readyState = OPEN;
        ...
        var onFinish = function (error) {
            if (currentState === OPEN || currentState === CONNECTING) {
                // JSON 응답인 경우 완성된 데이터를 onerror로 전달
                if (isJsonResponse && textBuffer !== '') {
                    try {
                        var jsonData = JSON.parse(textBuffer);
                        // ErrorEvent에 data 속성 추가하여 전달
                        var errorEvent = new ErrorEvent('error', {
                            error: null,
                        });
                        // JSON 데이터를 이벤트 객체에 직접 추가
                        errorEvent.data = JSON.stringify(jsonData);
                        errorEvent.status = 200;
                        errorEvent.isJsonResponse = true;

                        es.dispatchEvent(errorEvent);
                        fire(es, es.onerror, errorEvent);

                        // JSON 응답은 한 번만 받으므로 연결 종료
                        close();
                        return;
                    } catch (parseError) {
                        console.error('Failed to parse JSON response:', parseError);
                        // JSON 파싱 실패시 원본 텍스트를 그대로 전달
                        var errorEvent = new ErrorEvent('error', {
                            error: parseError,
                        });
                        errorEvent.data = textBuffer;
                        errorEvent.status = 200;
                        errorEvent.isJsonResponse = true;
