---
trigger_keywords: ['vite', 'build', '빌드', '번들', 'bundle']
trigger_globs: ['config/vite/**', 'config/env/**']
---

# Bundle & Execution Flow

CSR Boilerplate의 빌드 도구는 Vite 이며, 설정은 `config/vite/` 에 모여 있다.
환경 변수는 `config/env/index.ts` 에서 Zod로 검증된 뒤 Vite에 주입된다.

디렉토리 구조는 `tree config/` 또는 `ls config/vite/plugins/` 로 확인.
...
> HTTPS가 필요한 서드파티 SDK(카카오, 네이버 로그인 등)를 개발 중 테스트할 때 사용한다.
...
`public/` 디렉토리(`publicDir`)는 Vite가 네이티브로 관리한다.

| 경로 | 서빙 URL | 빌드 출력 | 특징 |
| ---- | -------- | --------- | ---- |
| `public/index.html` | `/index.html` | `dist/index.html` | vite-plugin-html이 EJS 처리 후 overwrite |
...
런타임에서는 `import.meta.env`를 직접 쓰지 않고 `@/shared/config/env`의 `ENV` 객체를 사용한다.

## HTML 진입점

단일 파일 `public/index.html`을 템플릿으로 사용한다. `vite-plugin-html`의 `createHtmlPlugin`이
mode에 따라 meta 태그와 EJS 치환을 처리한다.
...
HTML 내에서 env 값은 EJS 문법으로 참조한다: `<%= VITE_APP_TITLE %>`
...
- [spec/architecture/overview.md](./overview.md) — FSD 전체 레이어 구조
- [spec/api/client.md](../api/client.md) — axios 클라이언트 인터셉터
- [spec/env/config.md](../env/config.md) — 환경변수 상세 설명
