...
사실(구조) 전체는 [docs/spec/fsd-architecture/02-dependency-rules.md](/docs/spec/fsd-architecture/02-dependency-rules.md) 참조.
...
import { HomePage } from '@/pages/home'; // shared/features 에서 불가
```
...
```ts
// ✅ 상위 → 하위 레이어
import { apiClient } from '@/shared/api'; // pages, widgets, features 에서 모두 가능
import { LoginForm } from '@/features/auth'; // pages, widgets 에서 가능
...
- [ ] 레이어 결정: shared → features → widgets → pages 순으로 판단
